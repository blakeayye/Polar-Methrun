...
RegisterNetEvent('Polar-MethRun:client:clientChecks', function(data)
    ...
    if not coords then return QBCore.Functions.Notify(Config.CarInWay, 'error', 10000) end

    -- cooldown
    if Cooldown then  TriggerServerEvent('Polar-MethRun:server:StartJob', data.size, coords) end
...
exports[Config.Target]:AddGlobalVehicle({
    options = {
      {
        type = "client",
        event = "Polar-MethRun:client:DropOffPackage",
        icon = Config.IconLabelCar,
        label = Config.DropOffLabel,
        canInteract = function(entity)
            if not doinJob then return false end
            if not CachedNet then return false end
            if GetEntityModel(entity) ~= joaat(Config.MethVehicle) then return false end
            ...
            return canDropOffPackage(entity)
        end,
      }
    },
    distance = 1.0,
...
RegisterNetEvent('Polar-MethRun:client:OpenMainMenu', function()
    ...
    --[[if doinJob then
        menu[#menu+1] = {
            ...
            params = {
                isServer = true,
                event = "Polar-MethRun:server:CollectCheck",
            }
        }
    else]]
        menu[#menu+1] = {
            header = Config.MenuHeader2,
            txt = Config.MenuText,
            icon = Config.MenuHeaderIcon2,
            params = {
